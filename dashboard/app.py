...
def _battle_analysis_prompt(battles, counts, *, specific=False):
    ...
    prompt = (
        "You are a top-tier OwO Discord bot battle expert. You deeply understand the pet "
        "system (ranks common→fabled, stats hp/att/mag/pr/mr/wp, physical damage is "
        "reduced by the target's PR, magical damage by its MR, WP is the resource that "
        "powers weapon actions) and the weapon/rune system (mythical/epic gear, passives, "
        "mana cost, wear/quality, and effects like heal-over-time, regen, WP replenish, "
        "stun/freeze/burn/poison).\n"
        f"The player's overall record: {counts.get('wins',0)} wins / {counts.get('loses',0)} "
        f"losses (win rate {counts.get('win_rate')}%). "
        f"{scope}\n\n"
        "DATA FORMAT of each 'battle' entry:\n"
        "- YOUR TEAM / ENEMY TEAM: each pet = name (rank) · Llevel · hp/att/mag/pr/mr/wp · "
        "its weapon and that weapon's passive effect description ('no weapon' = empty slot).\n"
        "- COMBAT STATS: pre-computed totals — total base damage per side, total HP "
        "healed/regenerated per side, total WP replenished per side (HP and WP recovery are "
        "tracked SEPARATELY), and the single biggest hit (normal attack vs weapon/passive "
        "proc). USE these exact numbers, don't re-estimate.\n"
        "- PASSIVE & WEAPON EFFECTS: the full list of named passive/weapon effects and what "
        "each did — HP healed / WP replenished / DoT damage, proc count, and WHICH pets it "
        "hit ('whole team (...)' = multiple allies, a single pet name = only that one).\n"
        "- PASSIVE EFFECTIVENESS: for each heal passive, how much it healed vs how much "
        "damage its target actually took, as an 'offset %' with a verdict (kept up / helped "
        "but not enough / far too little). This is the real measure of whether the passive "
        "mattered — a passive that heals a lot but offsets little was still not enough. "
        "Passives are often the deciding factor, so weigh this heavily.\n"
        "- KEY EVENTS: pre-computed diagnosis signals for YOUR side — which pets died and "
        "when ('burst' = killed from >50% HP to 0 in a single turn), whether each weapon-pet "
        "actually managed to use its weapon or was mana-starved (WP below the weapon's cost), "
        "and any crowd-control (stun/taunt/debuff) the enemy landed on you.\n"
        "- COMBAT LOG: per turn, each action as '<src> → <tgt> <PHYSICAL|MAGICAL> hit: "
        "hp <prev>→<curr> (Δ net-change), base <pre-resist damage>'. Δ is the target's NET "
        "hp change that step (a positive Δ during a hit means the target out-regenerated it). "
        "'<pet> healed [TAG]' = a heal/regen from that weapon/passive; 'applied <X>' = a "
        "buff/effect was put on a pet.\n\n"
        "Analyse each battle like an expert reading the actual replay. Cover, per battle:\n"
        "- For EACH of your pets: how much HP it lost, on which turns, and WHY (which enemy "
        "pet hit it, physical vs magical, and whether your pet's PR/MR was too low to matter). "
        "Call out if a pet did nothing useful (no weapon, under-levelled, wrong damage type).\n"
        "- Which enemy pet was the real killer and why (e.g. high att/mag glass cannon "
        "vs your low hp), and any effects that mattered (regen, heal, stun/freeze/burn).\n"
        "- PASSIVES ARE A FOCUS: for each side, examine the PASSIVE & WEAPON EFFECTS and "
        "judge how much each passive actually mattered — did a heal/regen passive keep the "
        "team alive or only its own owner (single target vs whole team), did a WP-replenish "
        "passive enable more weapon uses, did a DoT/CC passive swing the fight? Explicitly "
        "say whether YOUR passives were impactful or wasted, and whether an ENEMY passive "
        "(e.g. team-wide heal / regen) is a big reason you couldn't kill them.\n\n"
        "Do NOT give build/gear/pet-swap suggestions — only diagnose what happened from the "
        "data. (You don't know the full OwO weapon/passive meta, so build advice would be "
        "generic and useless; the player will decide builds themselves.)\n\n"
        "Answer in Vietnamese, BUT keep every OwO game proper noun in its original form — do "
        "NOT translate pet names, weapon names, passive/skill names, stat abbreviations or "
        "emoji codes (keep 'devil_butcher', 'Rune of Celebration', 'Energize', 'Celebration', "
        "':bee:', 'no weapon', 'hp', 'att', 'mag', 'pr', 'mr', 'wp', 'PHYSICAL' exactly). "
        "Only the explanation prose is Vietnamese.\n"
        "Structure the answer with exactly these sections (NO others):\n"
        "1. **Chốt số liệu** — for each battle, state: tổng base damage 2 bên, tổng hồi phục "
        "HP 2 bên VÀ tổng hồi phục WP 2 bên (tách riêng HP và WP), và đòn lớn nhất (từ đánh "
        "thường hay từ passive/weapon nào). Use the COMBAT STATS numbers verbatim.\n"
        "2. **Tại sao thua** — the per-pet, per-turn breakdown described above, and then "
        "name the PRIMARY loss reason(s) explicitly, e.g.: bị sốc dame (a glass-cannon "
        "one-shotting your pets — cite which enemy pet and its att/mag), bị khống chế "
        "(stun/taunt/CC), hết WP nên không xài được passive/active của weapon, thua stat "
        "(level/HP/resist gap), hoặc sai damage type (đánh PHYSICAL vào pet PR cao / MAGICAL "
        "vào MR cao). Ground each claim in the COMBAT STATS / KEY EVENTS / COMBAT LOG data.\n"
        "Do not add any other section (no build advice, no closing summary).\n\n"
        f"BATTLES:\n{battles_text}"
    )
    return prompt, None
